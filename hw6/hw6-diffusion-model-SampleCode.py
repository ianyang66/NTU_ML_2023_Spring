...
class Dataset(Dataset):
    def __init__(
        ...
        self.transform = T.Compose([
            ...
            # T.RandomResizedCrop(128, scale=(0.5,1.0),ratio=(0.95,1.05)),
            T.RandomRotation(10),
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        dl = DataLoader(self.ds, batch_size = train_batch_size, shuffle = True, num_workers = 2, pin_memory = True)
        # cpu_count()
    ...
    def inference(self, num=200, n_iter=5, output_path='./submit'):
        if not os.path.exists(output_path):
            os.mkdir(output_path)
        with torch.no_grad():
            for i in range(n_iter):
                batches = num_to_groups(num // n_iter, 25)
                all_images = list(map(lambda n: self.ema.ema_model.sample(batch_size=n), batches))[0]
                for j in range(all_images.size(0)):
                    torchvision.utils.save_image(all_images[j], f'{output_path}/{i * 25 + j + 1}.jpg')              
...
# lr = 1e-3
# grad_steps = 4            # gradient accumulation steps, the equivalent batch size for updating equals to batch_size * grad_steps = 16 * 1
...
# timesteps = 100            # Number of steps (adding noise)
timesteps = 2500            # Number of steps (adding noise)
beta_schedule = 'linear'
...
trainer.train()
ckpt = './results4/model-20.pt'
trainer.load(ckpt)
trainer.inference(num=1000, n_iter=40)
...
for i in tmp:
    if count%5==0:
        torchvision.utils.save_image(i, f'results4/{count}.jpg')
    count+=1
...
# get_ipython().system('tar -zcf ../submission.tgz *.jpg')
